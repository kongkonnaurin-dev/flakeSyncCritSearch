...
public class CritSearchMojo extends FlakeSyncAbstractMojo {
    ...
    private long rootSearchTime = 0L;
    private long boundarySearchTime = 0L;
    private int totalExecutions = 0;
    private int rootExecutions = 0;
    private int boundaryExecutions = 0;
    private int rootCallStackDepth = 0;
    private int boundaryLinesChecked = 0;
    ...
    public void execute() throws MojoExecutionException, MojoFailureException {
        ...
        long startTotal = System.nanoTime();   // Total timer (added)
        ...
        try {
            ...
            for (String line : stackTraceLines) {
                ...
                for (int i = 0; i < locs.length - 1; i++) {
                    ...
                    // ---- PROFILING: count this root-search test execution ----
                    totalExecutions++;
                    rootExecutions++;
                    rootCallStackDepth++;
                    ...
                    if (!result) { // Test passes, we need to try longer delays
                        ...
                        while (workingDelay <= maxDelay) {
                            // ---- PROFILING: each retry is also a separate execution ----
                            totalExecutions++;
                            rootExecutions++;
                            rootCallStackDepth++;
            ...
            rootSearchTime += (System.nanoTime() - rootStart);   // Root search done
            // --- END ROOT SEARCH PHASE ---
            ...
            boundarySearchTime += (System.nanoTime() - boundaryStart);
            // --- END BOUNDARY SEARCH PHASE ---

            // --- PROFILING SUMMARY ---
            long endTotal = System.nanoTime();
            System.out.println("\n========== CRITSEARCH RESULTS ==========");
            System.out.println("Total time: " + (endTotal - startTotal) / 1_000_000 + " ms");
            System.out.println("Root search time: " + rootSearchTime / 1_000_000 + " ms");
            System.out.println("Critical point search time: " + boundarySearchTime / 1_000_000 + " ms");
            System.out.println("Root search iterations: " + rootCallStackDepth);
            System.out.println("=========================================");
        ...
        try {
            ...
            while (line != null) {
                ...
                for (int i = lowerLineNumber; i < upperLineNumber; i++) {
                    ...
                    // ---- PROFILING: count each boundary-line test execution ----
                    totalExecutions++;
                    boundaryExecutions++;
                    boundaryLinesChecked++;
